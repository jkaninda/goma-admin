    ...
    <div v-if="visible" class="modal-overlay" @click.self="handleCancel" @keydown="onKeydown">
      <div class="modal confirm-dialog" role="alertdialog" aria-modal="true">
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="handleCancel">{{ options.cancelText }}</button>
          <button class="btn" :class="options.variant === 'danger' ? 'btn-danger' : 'btn-primary'" @click="handleConfirm">
            {{ options.confirmText }}
          </button>
        </div>
      </div>
...
.confirm-header-content { display: flex; align-items: center; gap: 12px; }
.confirm-icon { display: flex; align-items: center; justify-content: center; width: 38px; height: 38px; border-radius: 10px; flex-shrink: 0; }
.confirm-icon-danger { background: var(--danger-50); color: var(--danger-600); }
.confirm-icon-warning { background: var(--warning-50); color: var(--warning-600); }
.confirm-icon-info { background: var(--primary-50); color: var(--primary-600); }
.confirm-message { font-size: 14px; color: var(--text-secondary); line-height: 1.6; }
</style>
